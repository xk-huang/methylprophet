...
# num_mp_workers=30 to reduce memory usage; < 500GB
...
python scripts/tools/data_preprocessing/convert_parquet_to_mds.py \
    ...
    --output_dir data/mds_tokenized/${PARQUET_DIR_NAME}-${CPG_SPLIT_NAME}-${SAMLE_SPLIT_NAME}/train \
...
python scripts/tools/data_preprocessing/convert_parquet_to_mds.py \
    ...
    --output_dir data/mds_tokenized/${PARQUET_DIR_NAME}-${CPG_SPLIT_NAME}-${SAMLE_SPLIT_NAME}/val \
    --num_workers "${NUM_WORKERS}" \
    --remove_unused_columns \
    --tokenize_sequence \
    --num_mp_workers 30
...
python scripts/tools/data_preprocessing/convert_parquet_to_mds.py \
    ...
    --output_dir data/mds_tokenized/${PARQUET_DIR_NAME}-${CPG_SPLIT_NAME}-${SAMLE_SPLIT_NAME}/val_10_shards \
    --num_workers 10 \
    --remove_unused_columns \
    --tokenize_sequence
...
python scripts/tools/data_preprocessing/convert_parquet_to_mds.py \
    ...
    --output_dir data/mds_tokenized/${PARQUET_DIR_NAME}-${CPG_SPLIT_NAME}-${SAMLE_SPLIT_NAME}/val_new_cpg \
    --num_workers "${NUM_WORKERS}" \
    --remove_unused_columns \
    --tokenize_sequence \
    --num_mp_workers 30
