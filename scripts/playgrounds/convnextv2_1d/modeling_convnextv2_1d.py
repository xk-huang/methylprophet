...
from .configuration_convnextv2_1d import ConvNextV21DConfig


logger = logging.get_logger(__name__)

# General docstring
_CONFIG_FOR_DOC = "ConvNextV21DConfig"
...
class ConvNextV1DropPath(nn.Module):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
class ConvNextV2GRN(nn.Module):
    ...
    def __init__(self, dim: int):
        super().__init__()
        # NOTE xk: hidden_states is of shape (batch_size, seq_len, num_channels)
...
class ConvNextV21DLayerNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.data_format == "channels_last":
            ...
            # NOTE xk: shape of x is (batch_size, channels, seq_len)
...
class ConvNextV2Embeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        if self.config.downsampling_in_stem:
            stride = config.patch_size
        else:
            stride = 1
        self.patch_embeddings = nn.Conv1d(
            config.num_channels, config.hidden_sizes[0], kernel_size=config.patch_size, stride=stride
        )
        self.layernorm = ConvNextV21DLayerNorm(config.hidden_sizes[0], eps=1e-6, data_format="channels_first")
...
class ConvNextV21DLayer(nn.Module):
    ...
    def __init__(self, config, dim, drop_path=0):
        ...
        self.layernorm = ConvNextV21DLayerNorm(dim, eps=1e-6)
        # pointwise/1x1 convs, implemented with linear layers
        self.pwconv1 = nn.Linear(dim, 4 * dim)
        self.act = ACT2FN[config.hidden_act]
        self.grn = ConvNextV2GRN(4 * dim)
        self.pwconv2 = nn.Linear(4 * dim, dim)
        self.drop_path = ConvNextV1DropPath(drop_path) if drop_path > 0.0 else nn.Identity()

    def forward(self, hidden_states: torch.FloatTensor) -> torch.Tensor:
        input = hidden_states
        x = self.dwconv(hidden_states)
        # NOTE xk: (batch_size, num_channels, seq_len) -> (batch_size, seq_len, num_channels)
        x = x.permute(0, 2, 1)
        x = self.layernorm(x)
        x = self.pwconv1(x)
        x = self.act(x)
        x = self.grn(x)
        x = self.pwconv2(x)
        # NOTE xk: (batch_size, seq_len, num_channels) -> (batch_size, num_channels, seq_len)
        x = x.permute(0, 2, 1)
...
class ConvNextV21DStage(nn.Module):
    ...
    def __init__(self, config, in_channels, out_channels, kernel_size=2, stride=2, depth=2, drop_path_rates=None):
        ...
        if in_channels != out_channels or stride > 1:
            self.downsampling_layer = nn.Sequential(
                ConvNextV21DLayerNorm(in_channels, eps=1e-6, data_format="channels_first"),
        ...
        self.layers = nn.Sequential(
            *[ConvNextV21DLayer(config, dim=out_channels, drop_path=drop_path_rates[j]) for j in range(depth)]
        )
...
class ConvNextV21DEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ConvNextV21DPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = ConvNextV21DConfig
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Conv2d):
            raise ValueError("This model is a 1D model, but a Conv2D layer was found.")
...
class ConvNextV21DModel(ConvNextV21DPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = ConvNextV2Embeddings(config)
        self.encoder = ConvNextV21DEncoder(config)
    ...
    def forward(
        ...
        # NOTE xk: global average pooling, (N, C, seq_len) -> (N, C)
        pooled_output = self.layernorm(last_hidden_state.mean(-1))
...
class ConvNextV21DForImageClassification(ConvNextV21DPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.num_labels = config.num_labels
        self.convnextv2 = ConvNextV21DModel(config)
...
class ConvNextV21DBackbone(ConvNextV21DPreTrainedModel, BackboneMixin):
    def __init__(self, config):
        super().__init__(config)
        ...
        for stage, num_channels in zip(self._out_features, self.channels):
            hidden_states_norms[stage] = ConvNextV21DLayerNorm(num_channels, data_format="channels_first")
