...
# Copied from: https://github.com/open-mmlab/mmengine/blob/9124ebf7a285aa785de77ad567aba06b5f276032/mmengine/utils/misc.py#L174
...
def _fill_missing_statistics(model: nn.Module, statistics: Dict[str, Dict[str, int]]) -> Dict[str, Dict[str, int]]:
...
def _stats_table_format(
    ...
    def fill(indent_lvl: int, prefix: str) -> None:
        ...
        for mod_name in statistics:
            # 'if mod' excludes root = '', which is never a child
            if mod_name and mod_name.count(".") == prefix.count(".") and mod_name.startswith(prefix):
...
def complexity_stats_str(flops: FlopAnalyzer, activations: Optional[ActivationAnalyzer] = None) -> str:
    ...
    all_uncalled = flops.uncalled_modules() | (activations.uncalled_modules() if activations is not None else set())
...
def complexity_stats_table(
    ...
    stats = _remove_zero_statistics(
        stats, require_trivial_children=True  # type: ignore
    )  # type: ignore
    ...
    param_shapes: Dict[str, Tuple[int, ...]] = {k: tuple(v.shape) for k, v in model.named_parameters()}
    to_delete = []
    for mod in stats:
...
def get_model_complexity_info(
    ...
    if inputs is None:
        ...
        if is_tuple_of(input_shape, int):  # tuple of int, construct one tensor
            ...
            [is_tuple_of(one_input_shape, int) for one_input_shape in input_shape]  # type: ignore
        ):  # tuple of tuple of int, construct multiple tensors
            inputs = tuple(
                [torch.randn(1, *one_input_shape).to(device) for one_input_shape in input_shape]  # type: ignore
            )
        else:
            raise ValueError(
