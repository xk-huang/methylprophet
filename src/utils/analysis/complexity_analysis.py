...
def activation_count(
    ...
    if supported_ops is None:
        supported_ops = {}
    act_counter = ActivationAnalyzer(model, inputs).set_op_handle(**supported_ops)
...
def parameter_count_table(model: nn.Module, max_depth: int = 3) -> str:
        ...
        for name, v in count.items():
            if name.count(".") == lvl and name.startswith(prefix):
                ...
                if name in param_shape:
                    rows.append((indent + name, indent + str(param_shape[name])))
                else:
    ...
    table = Table(title=f"parameter count of {model.__class__.__name__}", box=box.ASCII2)
